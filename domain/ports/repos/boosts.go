...
type BoostsRepository interface {
	...
	DeleteBoost(ctx context.Context, tx *db.Tx, localAccountID string, actor string, noteID string) error
	ListTimelineBoosts(ctx context.Context, tx *db.Tx, localAccountID string, limit int, maxID string) ([]models.Boost, error)
	ListActorBoosts(ctx context.Context, tx *db.Tx, localAccountID string, actor string, limit int, maxID string) ([]models.Boost, error)
	CountBoostsForNote(ctx context.Context, tx *db.Tx, noteID string) (int, error)
	BoostExists(ctx context.Context, tx *db.Tx, localAccountID string, actor string, noteID string) (bool, error)
}
