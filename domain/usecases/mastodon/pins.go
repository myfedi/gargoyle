...
func (u UseCase) PinStatus(ctx context.Context, account *models.Account, statusID string) (*TimelineItem, *domainerrors.DomainError) {
	return u.setStatusPin(ctx, account, statusID, true)
}

func (u UseCase) UnpinStatus(ctx context.Context, account *models.Account, statusID string) (*TimelineItem, *domainerrors.DomainError) {
	return u.setStatusPin(ctx, account, statusID, false)
}

func (u UseCase) setStatusPin(ctx context.Context, account *models.Account, statusID string, pinned bool) (*TimelineItem, *domainerrors.DomainError) {
	...
	if pinned {
		if _, err := u.cfg.SocialRepo.CreateInteraction(ctx, nil, account.ID, note.ID, "pin"); err != nil {
			return nil, domainerrors.NewErr(domainerrors.ErrInternal, err)
		}
	} else if err := u.cfg.SocialRepo.DeleteInteraction(ctx, nil, account.ID, note.ID, "pin"); err != nil {
		return nil, domainerrors.NewErr(domainerrors.ErrInternal, err)
	}
...
func (u UseCase) PinnedAccountStatuses(ctx context.Context, localAccount *models.Account, accountID string, limit int) ([]TimelineItem, *domainerrors.DomainError) {
