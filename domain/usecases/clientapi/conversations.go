...
func (u Conversations) Conversations(ctx context.Context, account *models.Account, limit int, maxID string) ([]ConversationItem, *domainerrors.DomainError) {
	...
	notes, err := u.deps.NotesRepo.ListDirectNotesPaged(ctx, nil, account.ID, limit*3, maxID)
	if err != nil {
...
func (u Conversations) conversationItem(ctx context.Context, account *models.Account, note models.Note, seen map[string]bool) (*ConversationItem, string, bool, *domainerrors.DomainError) {
	...
	dismissed, err := u.deps.ConversationsRepo.ConversationDismissed(ctx, nil, account.ID, id)
	if err != nil {
...
func (u Conversations) DismissConversation(ctx context.Context, account *models.Account, id string) *domainerrors.DomainError {
	if derr := requireAccount(account); derr != nil {
		return derr
	}
	if id == "" {
	...
	if err := u.deps.ConversationsRepo.DismissConversation(ctx, nil, account.ID, id); err != nil {
		return domainerrors.NewErr(domainerrors.ErrInternal, err)
	}
	return nil
}

func (u Conversations) ReadConversation(ctx context.Context, account *models.Account, id string) *domainerrors.DomainError {
	if derr := requireAccount(account); derr != nil {
		return derr
	}
	if id == "" {
...
func (u Conversations) conversationAccounts(ctx context.Context, local *models.Account, note models.Note) ([]models.Account, *domainerrors.DomainError) {
	...
	for _, match := range mentions {
		...
		if domain == strings.ToLower(u.deps.Domain) {
			acct, err := u.deps.AccountsRepo.GetLocalAccountByUsername(ctx, nil, username)
			if err == nil && acct.URI != local.URI {
				byURI[acct.URI] = *acct
			}
			continue
		}
		cached, err := u.deps.RemoteAccountsRepo.SearchRemoteAccounts(ctx, nil, username+"@"+domain, 1)
