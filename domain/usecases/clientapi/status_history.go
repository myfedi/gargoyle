...
func (u Statuses) StatusHistory(ctx context.Context, localAccount *models.Account, statusID string) ([]StatusEditHistoryItem, *domainerrors.DomainError) {
	item, derr := u.GetStatus(ctx, localAccount, statusID)
	if derr != nil {
		return nil, derr
	}
	edits, err := u.deps.NotesRepo.ListNoteEdits(ctx, nil, item.Note.ID)
	if err != nil {
...
func (u Statuses) mediaForHistoryEdit(ctx context.Context, mediaIDs []string) ([]models.MediaAttachment, *domainerrors.DomainError) {
	media := make([]models.MediaAttachment, 0, len(mediaIDs))
	for _, id := range mediaIDs {
		item, err := u.deps.MediaRepo.GetMediaAttachmentByID(ctx, nil, id)
		if err != nil {
			return nil, domainerrors.NewErr(domainerrors.ErrInternal, err)
		}
